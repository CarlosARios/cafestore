...
describe('CafeListComponent', () => {
  ...
  it('should create a table with a header and 4 records', () => {
    // Verifica que el encabezado de la tabla se ha creado
    const tableHeader = debug.query(By.css('thead'));
    expect(tableHeader).toBeTruthy();
  
    // Verifica que se han creado 4 registros
    const tableRows = debug.queryAll(By.css('tbody tr'));
    expect(tableRows).toHaveSize(4);
  });
});
